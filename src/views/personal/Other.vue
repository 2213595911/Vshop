<template>
  <div v-if="path === '帮助中心'">
    <van-list finished-text="没有更多了">
      <van-cell v-for="(item, index) in helps" :key="item" :title="index + 1 + '   ·' + item.title" />
    </van-list>
  </div>
  <div v-if="path === '我的订单'">
    <template v-if="$store.state.personal?.isLogin">
      <div class="order_list" v-if="uesrInfo.orders.length">
        <div class="list clear" v-for="(item, index) in uesrInfo.orders" :key="index">
  ...
  <div v-if="path === '我的地址'">我的地址</div>
</template>

<script setup lang="ts">
import { watch, ref, reactive, computed, ComputedRef, forceUpdate } from 'vue'
import { useRoute } from 'vue-router'
import { useStore } from 'vuex'
import { key } from '@/store'
import type { helpType, userType } from '@/types/usePersonal'
...
// 订单列表
const uesrInfo: ComputedRef<userType> = computed(() => {
